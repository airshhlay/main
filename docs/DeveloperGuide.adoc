...
The following sequence diagram illustrates how the `LogicToUiManager` can be used to modify the `MainWindow`, when CardiBuddy needs to display the flashcard question to the user during a `Test Session`. More details on `Test Session` in the section: <<test-sessions, here>>.
...
Go to <<Design-Model, Model>> to view the full class diagram for the `Model` component, to see all the classes within `Model` interact.
